...
table=$(
  ...
  echo "    2a  vanilla(sonnet)+PR-subs(sonnet) vs vanilla(sonnet)+vanilla-subs(sonnet)     — Do pyramid-reader subagents beat vanilla subagents with vanilla Claude? (skill off both sides)"
  echo "    2b  vanilla(sonnet)+PR-subs(haiku)  vs vanilla(sonnet)+vanilla-subs(haiku)      — Do pyramid-reader subagents beat vanilla subagents with vanilla Claude? (haiku tier, skill off both sides)"
  echo "    2c  vanilla(haiku)+PR-subs(haiku)  vs vanilla(sonnet)+vanilla-subs(sonnet)         — Can a haiku+PR-subs setup match vanilla sonnet+subs quality/cost? (skill off both sides)"
  ...
  echo "    3a  skill(sonnet)+PR-subs(sonnet)  vs vanilla(sonnet)+vanilla-subs(sonnet)         — Does the full pyramid-read stack beat a fully-vanilla setup? (sonnet)"
  echo "    3b  skill(sonnet)+PR-subs(haiku)   vs vanilla(sonnet)+vanilla-subs(haiku)          — Does the full pyramid-read stack beat a fully-vanilla setup? (haiku)"
